...
* 三項演算子や内包表記は一目で何をしているのか判別できるケースのみに利用し、ネストが含まれるような複雑なケースでは利用しない
...
* レビューは必ず日本語で行うこと
* コードの変更の際、直接GitHubに対してコミットをせず、ローカルにて変更、コミット、pushすること


### Python import

* 公開クラス・関数・型は、利用側から見た責務と依存境界に応じて適切な`__init__.py`で再エクスポートし、`__all__`へ明示する
* 公開境界は原則として`application`、`domain`、`infra`など責務を持つレイヤー内に置き、それより外側のモジュール直下へ再エクスポートしない
...
- 複数領域にまたがる変更は、自分で小さな実装単位に分割して進める。
- 各単位では関連テストのみ実行し、全体検証は最後に1回行う。
- 実装方針が決まった後は、設計全体を再探索しない。
- 今回の変更と無関係な失敗やリファクタリングには手を広げない。
- 同じ失敗に対する同一アプローチの再試行を繰り返さない。
